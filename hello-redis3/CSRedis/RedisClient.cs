...
namespace CSRedis
{
    ...
    public partial class RedisClient : IRedisClientSync, IRedisClientAsync
    {
        ...
        public RedisClient(string host, int port, bool ssl, int asyncConcurrency, int asyncBufferSize)
            ...
            //: this(new IPEndPoint(Convert.ToByte(host), port), ssl, asyncConcurrency, asyncBufferSize)
            : this(new IPEndPoint(IPAddress.Parse(host), port), ssl, asyncConcurrency, asyncBufferSize)
        { }
